...
(function () {
  ...
  var SPLITS = {
    ...
    listen: [
      { clip: 'inset(0 0 43% 0)',      from: 'translate(34px,-26px) rotate(4deg)',  idle: 'i-drift', dur: '4.6s', del: '0s', free: true },
  ...
  /* スマホは画面が狭く、イラストも右端に置いているので、
     登場時にパーツが飛び出す量をそのままにすると画面の外にはみ出して
     横スクロールが生まれてしまう。幅が狭いときだけ移動量を小さくする。 */
  var isNarrow = window.matchMedia('(max-width: 767px)').matches;

  function scaleFrom(s) {
    if (!s || s === 'none') return 'none';
    if (!isNarrow) return s;
    return s.replace(/(-?\d+(?:\.\d+)?)px/g, function (m, n) {
      return Math.round(parseFloat(n) * 0.45) + 'px';
    });
  }
  ...
  function buildSplit(img) {
    ...
    parts.forEach(function (p, i) {
      ...
      wrapEl.style.setProperty('--from', scaleFrom(p.from));
    ...
    var fl = FLOAT[key] || ['', '3.5s', '0s'];
    if (fl[0]) fig.classList.add(fl[0]);
    fig.style.setProperty('--fdur', fl[1]);
    fig.style.setProperty('--fdel', fl[2]);
  ...
  /* ---------------------------------------------------------
     2. 画面に入るたびに動かす（IntersectionObserver）
        ・revealIO … 少し中まで入ったら再生
        ・resetIO  … 完全に画面の外へ出たら巻き戻す
        2つに分けているのは、画面のふちで再生と巻き戻しが
        ちらつかないようにするため。
     --------------------------------------------------------- */
  var targets = Array.prototype.slice.call(document.querySelectorAll('[data-anim], .fig'));

  function reveal(el) {
    if (el.classList.contains('in')) return;
    el.classList.add('in');
    if (el.classList.contains('fig')) {
      if (el._idleTimer) window.clearTimeout(el._idleTimer);
      el._idleTimer = window.setTimeout(function () {
  ...
  if ('IntersectionObserver' in window) {
    ...
    var resetIO = new IntersectionObserver(function (entries) {
      entries.forEach(function (e) {
        if (!e.isIntersecting) unreveal(e.target);
      });
    }, { rootMargin: '0px', threshold: 0 });

    targets.forEach(function (el) { revealIO.observe(el); resetIO.observe(el); });
  ...
  function countUp(el) {
    ...
    var t0 = null;
    function step(ts) {
      if (el._countToken !== token) return;   /* 巻き戻されたら中断 */
      if (t0 === null) t0 = ts;
  ...
  if ('IntersectionObserver' in window) {
    var numIO = new IntersectionObserver(function (entries) {
      entries.forEach(function (e) { if (e.isIntersecting) countUp(e.target); });
    }, { threshold: 0.6 });

    var numResetIO = new IntersectionObserver(function (entries) {
      entries.forEach(function (e) { if (!e.isIntersecting) countReset(e.target); });
    }, { rootMargin: '0px', threshold: 0 });

    nums.forEach(function (el) { numIO.observe(el); numResetIO.observe(el); });
  } else {
  ...
  var typeTimer = null, typeDelay = null, caretTimer = null;

  function typeStop() {
    if (typeTimer) { window.clearInterval(typeTimer); typeTimer = null; }
    if (typeDelay) { window.clearTimeout(typeDelay); typeDelay = null; }
    if (caretTimer) { window.clearTimeout(caretTimer); caretTimer = null; }
  }
  ...
  if (q) {
    ...
    function typeStart() {
      ...
      q.textContent = '';
      typeDelay = window.setTimeout(function () {
        var i = 0;
        typeTimer = window.setInterval(function () {
          ...
          if (i >= text.length) {
            ...
            typeTimer = null;
            caretTimer = window.setTimeout(function () {
              if (caret) caret.classList.add('is-done');
            }, 1800);
          }
        }, 170);
    ...
    if ('IntersectionObserver' in window) {
      var searchEl = q.closest('.search') || q;
      var typeIO = new IntersectionObserver(function (entries) {
        entries.forEach(function (e) { if (e.isIntersecting) typeStart(); });
      }, { threshold: 0.4 });

      var typeResetIO = new IntersectionObserver(function (entries) {
        entries.forEach(function (e) { if (!e.isIntersecting) typeReset(); });
      }, { rootMargin: '0px', threshold: 0 });

      typeIO.observe(searchEl);
      typeResetIO.observe(searchEl);
    } else {
      q.textContent = text;
    }
  }
  ...
  if (toolsBtn && toolsPanel) {
    ...
    toolsPanel.addEventListener('click', function (e) {
      if (e.target.closest('a')) closeTools();
    });
  }
